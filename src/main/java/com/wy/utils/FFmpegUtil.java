...
//
///**
// * @apiNote java调用ffmpeg,该工具在本工程中暂无使用
// * <dependency>
...
//
//	/**
//	 * 视频帧抽取时的默认时间点，第10s（秒） （Time类构造参数的单位:ms）
//	 */
...
//
//	/**
//	 * 视频时长正则匹配式 用于解析视频及音频的时长等信息时使用； (.*?)表示：匹配任何除\r\n之外的任何0或多个字符，非贪婪模式
//	 */
...
//		if (StrUtils.isBlank(ffmpeg_path)) {
...
//		if (StrUtils.isBlank(ffmpegVersionStr)) {
...
//			String resultStr = StrUtils.isBlank(result) ? "【异常】" : "正常";
...
//
//	/**
//	 * 视频帧抽取（抽取指定时间点、指定宽度值、指定高度值的帧画面） 转换后的文件路径以.gif结尾时，默认截取从指定时间点开始，后10s以内的帧画面来生成gif
//	 * @param videoFile 源视频路径
//	 * @param fileOutPut 转换后的文件路径
//	 * @param time 指定要抽取第几秒的视频帧（单位：s）
//	 * @param width 抽取的视频帧图片的宽度（单位：px）
...
//			String tempPath = path + File.separator + System.currentTimeMillis() + "_"
...
//	 * @param isContinuty false - 静态图（只截取time时间点的那一帧图片），true -
//	 *        动态图（截取从time时间点开始,timelength这段时间内的多张帧图）
//	 */
//	private static void cutVideoFrame(File videoFile, String path, Time time, int width, int height, int timeLength,
//			boolean isContinuty) {
...
//	 * @param timeLength
//	 *        需要抽取的时间段（单位:s，需小于源视频最大时长）；例如：该参数值为10时即抽取从startTime开始之后10秒内的视频作为新视频
//	 */
...
//
//	/**
//	 * 解析视频的基本信息（从文件中） 解析出的视频信息一般为以下格式： Input #0, mov,mp4,m4a,3gp,3g2,mj2, from
//	 * '6.mp4': Duration: 00:00:30.04, start: 0.000000, bitrate: 19031 kb/s Stream
//	 * #0:0(eng): Video: h264 (Main) (avc1 / 0x31637661), yuv420p(tv, bt709),
//	 * 1920x1080, 18684 kb/s, 25 fps, 25 tbr, 25k tbn, 50 tbc (default) Stream
//	 * #0:1(eng): Audio: aac (LC) (mp4a / 0x6134706D), 48000 Hz, stereo, fltp, 317
//	 * kb/s (default) 注解： Duration: 00:00:30.04【视频时长】, start: 0.000000【视频开始时间】,
//	 * bitrate: 19031 kb/s【视频比特率/码率】 Stream #0:0(eng): Video: h264【视频编码格式】 (Main)
//	 * (avc1 / 0x31637661), yuv420p(tv, bt709), 1920x1080【视频分辨率，宽x高】, 18684【视频比特率】
//	 * kb/s, 25【视频帧率】 fps, 25 tbr, 25k tbn, 50 tbc (default) Stream #0:1(eng):
//	 * Audio: aac【音频格式】 (LC) (mp4a / 0x6134706D), 48000【音频采样率】 Hz, stereo, fltp,
